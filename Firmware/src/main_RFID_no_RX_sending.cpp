...
TX_RETURN_TYPE SENDdata(uint8_t port, bool cnf = false)
{
  ...
  {
    DEBUG_PRINTLN("Send without confirmation");
  }
  ...
  {  
    ...
    else
    {
      ...
      DEBUG_PRINT(F("stop confirmation: ")); DEBUG_PRINTLN(timeToSleep);
...
void sendData(unsigned long frequency, uint8_t port,  bool confirmation) // frequency in ms
{
  if (millis() >= sendStartTime + frequency)
  {
    ...
    SENDdata(port, confirmation); //sending, port can be specified
...
void setup()
{
  ...
  confirmation = true; //sending with confirmation, and thus also the ability of receiving messages
...
void loop()
{
  ...
  if (FlowerState == wakeUp)
  {
    ...
    DEBUG_PRINT("Start IR: "); DEBUG_PRINTLN(previousSensorValue);
  ...
  if (FlowerState == work)
  {
    if (timeToSleep == true) // keep sending data, but stop waiting for confirmation (before switching on RFID readers)
    { 
      confirmation = false;
    }
    ...
    sendData(WORK_SEND_FREQUENCY, 1, confirmation);   // send data over LoRaWAN (e.g. every 10min.), port 1
